              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className={`group relative transition-all duration-300 ${isFocused === 'password' ? 'translate-x-1' : ''}`}>
                    <input
                      type={showPassword ? "text" : "password"} // Type diubah berdasarkan state
                      value={password}
                      onChange={(e) => setPassword(e.target.value)}
                      onFocus={() => setIsFocused('password')}
                      onBlur={() => setIsFocused(null)}
                      className={`w-full pl-6 pr-14 py-4 bg-slate-50 border-2 rounded-2xl outline-none text-slate-900 font-bold transition-all ${isFocused === 'password' ? 'border-red-600 bg-white shadow-xl shadow-red-600/5' : 'border-transparent hover:bg-slate-100'
                        }`} // pr-14 ditambahkan agar text tidak menabrak ikon mata
                      placeholder="••••••••"
                      required
                    />
