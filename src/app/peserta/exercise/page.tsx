        ...
        <AnimatePresence>
          {selectedLesson && (
            <motion.div 
              ...
              className="max-w-3xl mx-auto bg-white rounded-2xl md:rounded-[2rem] border-2 border-[#1e4e8c] shadow-xl overflow-hidden mb-8 md:mb-10"
            >
              <div className="bg-slate-50 px-4 md:px-6 py-3 border-b border-slate-100 flex justify-between items-center">
                <span className="text-[10px] md:text-xs font-black bg-[#1e4e8c] text-white px-2 md:px-3 py-1 rounded-md uppercase tracking-tighter">
                  {selectedLesson.type}
                </span>
                <button 
              ...
              <div className="p-4 md:p-6">
                <h2 className="text-lg md:text-xl font-black text-slate-900 mb-3 md:mb-4 uppercase italic leading-tight">
                  {selectedLesson.title}
                </h2>
                
                <div className="aspect-video max-h-[250px] md:max-h-[350px] w-full bg-slate-900 rounded-xl md:rounded-2xl flex items-center justify-center text-white mb-5 md:mb-6 shadow-inner relative overflow-hidden">
                  <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]"></div>
                  <p className="font-bold text-[10px] md:text-xs uppercase tracking-[0.2em] md:tracking-[0.3em] opacity-40 italic animate-pulse z-10 text-center px-4">
                    Playing {selectedLesson.type} Content...
                  </p>
                </div>

                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0">
                  <p className="text-[10px] md:text-[11px] text-slate-400 font-medium italic">
                    Pastikan Anda memahami materi sebelum melanjutkan.
                  </p>
                  <button 
                    onClick={handleCompleteLesson} 
                    className="w-full sm:w-auto justify-center bg-[#c31a26] text-white px-6 py-3 rounded-xl font-black text-sm shadow-md hover:shadow-red-200 hover:-translate-y-0.5 active:scale-95 transition-all uppercase flex items-center gap-2"
                  >
        ...
        <div className="bg-[#f8fafc] rounded-3xl md:rounded-[2.5rem] p-4 sm:p-6 md:p-8 border border-slate-200 shadow-sm relative">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 md:mb-8 gap-4 sm:gap-0">
            ...
            <button 
                ...
                className={`w-full sm:w-auto text-center ${progress < 100 ? 'bg-slate-300' : 'bg-[#1e4e8c]'} text-white px-5 md:px-6 py-3 md:py-2.5 rounded-xl font-black text-[10px] md:text-xs uppercase shadow-lg transition-colors`}
            >
              Lihat Sertifikat
            </button>
          </div>
          ...
          {activeModule === 1 ? (
            /* ================= MODUL 1 ================= */
            <>
              {/* TAMPILAN MOBILE & IPAD: Bersampingan berurutan (Buku 1, Video 1, Buku 2, dst) */}
              ...
              {/* TAMPILAN LAPTOP: Sesuai Kode Asli (Zig Zag Atas Bawah) */}
              ...
              <div className="flex justify-end mt-8 md:mt-10">
                {progress >= 100 && (
                  <button onClick={() => goToModule(2)} className="text-[#1e4e8c] font-black hover:text-blue-800 transition-all text-[10px] md:text-xs uppercase italic tracking-tighter bg-blue-50 md:bg-transparent px-4 py-2 md:p-0 rounded-lg md:rounded-none">
                    Lanjut ke Modul 2 →
                  </button>
                )}
              </div>
            ...
            /* ================= MODUL 2 ================= */
            <>
              {/* TAMPILAN MOBILE & IPAD: Bersampingan berurutan */}
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mb-6 md:mb-10 lg:hidden">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center mt-8 md:mt-10 gap-4 sm:gap-0">
                <button onClick={() => goToModule(1)} className="text-slate-400 font-bold hover:text-slate-600 transition-all text-[10px] md:text-xs uppercase italic bg-slate-100 md:bg-transparent px-4 py-2 md:p-0 rounded-lg md:rounded-none w-full sm:w-auto text-center">
                  ← KEMBALI KE MODUL 1
                </button>
                {progress >= 100 && (
                  <button onClick={() => goToModule(3)} className="text-[#1e4e8c] font-black hover:text-blue-800 transition-all text-[10px] md:text-xs uppercase italic tracking-tighter bg-blue-50 md:bg-transparent px-4 py-2 md:p-0 rounded-lg md:rounded-none w-full sm:w-auto text-center">
                    Lanjut ke Modul 3 →
                  </button>
                )}
              </div>
            ...
            <>
              ...
              <div className="flex justify-start mt-8 md:mt-10">
                <button onClick={() => goToModule(2)} className="text-slate-400 font-bold hover:text-slate-600 transition-all text-[10px] md:text-xs uppercase italic bg-slate-100 md:bg-transparent px-4 py-2 md:p-0 rounded-lg md:rounded-none w-full sm:w-auto text-center">
                  ← KEMBALI KE MODUL 2
                </button>
              </div>
            </>
