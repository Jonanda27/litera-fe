...
// Komponen VideoCard dengan Memo agar tidak re-render terus menerus
const VideoCard = memo(({ id, isLocal, name, stream, isCamOn, isMicOn, isScreenSharing, pinnedId, setPinnedId, isHandRaised, customClass = "" }) => {
    ...
    useEffect(() => {
        if (videoRef.current && stream) {
            ...
            videoRef.current.onloadedmetadata = () => {
                videoRef.current.play().catch(e => console.error("Autoplay failed:", name, e));
            };
        }
    }, [stream, name]);
    ...
    return (
        <div className={`relative group rounded-2xl overflow-hidden bg-neutral-900 border-2 border-white/5 transition-all duration-300 shadow-xl ${customClass}`}>
            ...
            {isHandRaised && (
                <div className="absolute top-2 right-10 sm:top-3 sm:right-14 bg-yellow-500/90 backdrop-blur-md px-2 py-1 sm:py-1.5 rounded-lg sm:rounded-xl text-white z-30 animate-bounce shadow-lg text-[10px] sm:text-base">✋</div>
            )}
...
export default function WebRTCMeeting({ roomId }) {
    ...
    const createPeerConnection = useCallback(async (targetId, isInitiator, targetName = "User") => {
        ...
        pc.ontrack = (event) => {
            setRemoteStreams(prev => {
                ...
                return [...prev, { id: targetId, stream: event.streams[0], name: targetName }];
            });
        };
    ...
    const initSocket = useCallback(() => {
        ...
        socket.on("webrtc_offer", async ({ offer, senderId, senderName }) => {
            try {
                const pc = await createPeerConnection(senderId, false, senderName);
            ...
            <div className="fixed inset-0 bg-black flex items-center justify-center p-4 text-white z-[99999]">
                <div className="w-full max-w-md bg-neutral-900 p-8 rounded-[2.5rem] border border-white/10 shadow-2xl">
                    ...
                    <form onSubmit={handleJoin} className="space-y-6">
                        ...
                        <button type="submit" className="w-full bg-blue-600 py-4 rounded-xl font-black text-lg active:scale-95">Bergabung</button>
                    </form>
                </div>
            </div>
        );
            ...
            <div className="flex-1 flex flex-col relative h-full overflow-hidden">
                ...
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[10006] w-max">
                    <div className="bg-[#1e1e1e]/90 backdrop-blur-3xl p-4 px-8 rounded-full flex items-center gap-5 border border-white/10 shadow-2xl">
                        ...
                        <button onClick={() => window.location.reload()} className="bg-red-600 text-white px-6 py-3 rounded-full font-bold active:scale-95">Keluar</button>
                    </div>
                </div>
            </div>
                    ...
                    {isChatOpen && (
                        <div className="flex flex-col h-full">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-4">
                                {messages.map((msg, idx) => (
                                    <div key={idx} className={`flex flex-col ${msg.senderId === socket.id ? 'items-end' : 'items-start'}`}>
                                        <span className="text-[10px] font-bold text-blue-400 mb-1">{msg.senderName}</span>
                                        <div className={`px-4 py-2.5 rounded-2xl text-sm max-w-[90%] break-words ${msg.senderId === socket.id ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white/10 text-white rounded-tl-none'}`}>
                                            {msg.text}
                                        </div>
                                    </div>
                                ))}
                            ...
                            <form onSubmit={sendChat} className="p-4 border-t border-white/5 bg-[#1a1a1a]">
                                <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Tulis pesan..." className="w-full bg-white/5 border border-white/10 rounded-2xl py-3.5 px-5 text-sm outline-none" />
                            </form>
                        </div>
                    )}
