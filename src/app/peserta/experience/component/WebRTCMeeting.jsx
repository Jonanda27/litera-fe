...
export default function WebRTCMeeting({ roomId }) {
    ...
    }, [remoteStreams, pinnedId, isScreenSharing, layoutType, isLayoutModalOpen, isCamOn, isMicOn, isParticipantsOpen]);
    ...
    const initSocket = () => {
        ...
        socket.on("video_user_joined", ({ socketId, name }) => {
            ...
            addNotification(`${name || 'Seseorang'} telah bergabung ke rapat`);
        ...
        socket.on("video_user_left", (id) => {
            ...
            addNotification(`${leftName} telah meninggalkan rapat`);
    ...
    const VideoCard = ({ id, isLocal, isPinned, customClass = "" }) => {
        ...
        return (
            <div className={`relative group rounded-2xl overflow-hidden bg-neutral-900 border-4 transition-all duration-500 ${customClass}
                ...
                <video key={`vid-${id}`} ref={el => { if (isLocal) localVideoRef.current = el; else if (el) videoRefs.current[id] = el; }} autoPlay muted={isLocal} playsInline className="w-full h-full object-cover bg-neutral-900" />
                ...
                <button onClick={() => setPinnedId(pinnedId === id ? null : id)} className={`absolute top-3 right-3 p-2 rounded-full transition-all z-20 ${pinnedId === id ? 'bg-blue-600 text-white' : 'bg-black/40 text-white opacity-0 group-hover:opacity-100 hover:bg-black/60'}`}>📌</button>
    ...
    return (
        <div className="w-full h-screen bg-black flex flex-col overflow-hidden relative">
            ...
            {/* CONTAINER UNTUK NOTIFIKASI (POJOK KIRI BAWAH) */}
            <div className="fixed bottom-28 left-6 z-[10001] flex flex-col gap-2 pointer-events-none">
                {notifications.map(n => (
                    <div key={n.id} className="bg-neutral-900/90 text-white px-4 py-3 rounded-xl border border-white/10 shadow-2xl backdrop-blur-md flex items-center gap-3 animate-in slide-in-from-left duration-300 pointer-events-auto">
            ...
            <div className="flex flex-1 overflow-hidden relative">
                <div className="flex-1 overflow-hidden p-4 sm:p-6 relative transition-all duration-300">
                    {layoutType === 'grid' && (
                ...
                <div className={`transition-all duration-300 bg-neutral-900 border-l border-white/10 flex flex-col ${isParticipantsOpen ? 'w-80 opacity-100' : 'w-0 opacity-0 overflow-hidden'}`}>
                ...
                <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white text-neutral-900 w-full max-w-sm rounded-3xl p-6 relative shadow-2xl">
                        ...
                        <div className="space-y-4">
                            {[{ id: 'auto', name: 'Otomatis (dinamis)', icon: '✦' }, { id: 'grid', name: 'Kotak (lama)', icon: '▦' }, { id: 'focus', name: 'Sorotan', icon: '▢' }, { id: 'sidebar', name: 'Sidebar', icon: '◫' }].map((item) => (
            ...
            <div className="fixed bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-black via-black/80 to-transparent flex items-center justify-center z-[9999]">
                <div className="flex items-center gap-2 sm:gap-4 bg-neutral-900/90 backdrop-blur-2xl p-3 sm:p-4 px-6 sm:px-10 rounded-full border border-white/10 shadow-2xl mb-4">
                    ...
                    <button onClick={() => setIsParticipantsOpen(!isParticipantsOpen)} className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full transition-all ${isParticipantsOpen ? 'bg-blue-600' : 'bg-neutral-800'}`}>
                        <span className="text-xl">👥</span>
                    </button>
                    <button onClick={() => setIsLayoutModalOpen(true)} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-neutral-800 hover:bg-neutral-700 transition-all text-white">
