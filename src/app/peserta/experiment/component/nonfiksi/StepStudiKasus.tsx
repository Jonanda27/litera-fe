...
export default function StepCaseStudy({ formData, onDataChange }: StepCaseStudyProps) {
  ...
  return (
    <div className="space-y-6 pb-10 text-slate-800 relative px-1 md:px-0">

      {/* HEADER SECTION */}
      <div className="flex flex-col sm:flex-row justify-between items-center bg-gradient-to-r from-indigo-600 to-blue-700 p-4 md:p-5 rounded-2xl md:rounded-[2rem] shadow-lg gap-4">
        ...
        <button
          ...
          className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase transition-all shadow-md active:scale-95 flex items-center gap-2 ${isAdding ? "bg-rose-500 text-white shadow-rose-200" : "bg-white text-indigo-700 hover:bg-indigo-50"
            }`}
        >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {caseTypes.map((t) => (
                      <button key={t} type="button" onClick={() => setNewItem({ ...newItem, type: t })}
                        className={`py-2 px-3 md:px-4 rounded-full text-[8px] md:text-[9px] font-black uppercase transition-all border ${newItem.type === t ? "bg-indigo-600 border-indigo-600 text-white shadow-md" : "bg-slate-50 border-slate-100 text-slate-500 hover:border-indigo-200"
                          }`}
                      >
                        {t}
                      </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {cases.map((item) => (
          <motion.div layout key={item.id} onClick={() => setSelectedCase(item)}
            className="bg-white p-4 md:p-6 rounded-2xl md:rounded-[2.5rem] border-2 border-slate-100 shadow-sm hover:shadow-xl hover:border-indigo-400 transition-all group relative overflow-hidden cursor-pointer"
          >
        ...
        {selectedCase && (
          ...
          >
            <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }}
              className="bg-white w-full max-w-3xl rounded-t-2xl sm:rounded-[3rem] shadow-2xl overflow-hidden text-slate-800 flex flex-col max-h-[95vh] sm:max-h-[90vh]"
            ...
            >
              ...
              <div className="p-5 md:p-8 overflow-y-auto space-y-8 md:space-y-10 custom-scrollbar text-black">
      ...
      @media (min-width: 768px) {
        .form-input-modal { border-radius: 12px; }
      }
