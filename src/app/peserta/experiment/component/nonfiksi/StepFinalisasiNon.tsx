...
// Tambahkan properti pageNumber untuk mendeteksi halaman ke-berapa di dalam bab tersebut
...
const StepFinalisasiNon = forwardRef((props: StepFinalisasiProps, ref) => {
  ...
  useEffect(() => {
    const fetchFullBook = async () => {
      ...
      setIsLoading(true);
      try {
        // Ambil struktur bab untuk Non-Fiksi
        const outlineRes = await axios.get(`${API_BASE_URL}/books/chapter-structures/${bookId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const chapters = outlineRes.data;

        // Ambil konten per bab
        const contentPromises = chapters.map((chap: any) =>
          axios.get(`${API_BASE_URL}/books/non-fiction/get-content?bookId=${bookId}&chapterNumber=${chap.chapterNumber}`, {
            headers: { Authorization: `Bearer ${token}` },
          })
        );
        ...
        contentsResults.forEach((res, index) => {
          ...
          // Hasil data sekarang berupa array of pages dari backend
          const pagesData = res.data.data; 

          if (Array.isArray(pagesData) && pagesData.length > 0) {
            // Jika datanya array (Multi-page format baru)
            pagesData.forEach((pageItem: any) => {
              flattenedPages.push({
                chapterTitle: chapter.title,
                chapterNumber: chapter.chapterNumber,
                pageNumber: pageItem.pageNumber || 1,
                content: pageItem.content || "",
                pageIndex: globalPageIndex++, 
              });
            });
          } else if (pagesData && typeof pagesData === 'object' && pagesData.content) {
            // Fallback: Jika masih menggunakan format lama (single object string)
            flattenedPages.push({
              chapterTitle: chapter.title,
              chapterNumber: chapter.chapterNumber,
              pageNumber: 1,
  ...
  const handleDownloadOnly = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', `${formData.title || 'naskah_nonfiksi'}.pdf`);
    ...
    <>
      ...
      {allPages.map((page, index) => (
        <div key={index} className="flex-shrink-0 snap-center flex flex-col items-center gap-6">
          {/* Label di atas kertas menampilkan Nomor Bab dan Hal ke berapa dalam bab tersebut */}
          <p className="text-[11px] font-black uppercase text-white bg-slate-900/50 px-4 py-1.5 rounded-full backdrop-blur-md">
            Bab {page.chapterNumber}: {page.chapterTitle} {page.pageNumber > 1 ? `(Hal. ${page.pageNumber})` : ''}
          </p>
          <div 
            ...
            style={{ 
              width: '210mm', minHeight: '297mm', height: 'auto', padding: '2.54cm',
              fontSize: previewConfig.fontSize || '12pt', lineHeight: '1.8', 
  ...
  return (
    <div className="flex flex-col h-full space-y-6 text-black">
      <div className="bg-slate-900 p-5 rounded-2xl text-white flex flex-col md:flex-row justify-between items-center shadow-xl gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {/* Mengganti label agar relevan: Berapa total lembar yang dirender */}
      ...
      <style jsx global>{`
        ...
        .a4-page-div p { margin-bottom: 1em !important; line-height: 1.8; text-align: justify; color: black; word-break: break-word; }
...
StepFinalisasiNon.displayName = "StepFinalisasiNon";
export default StepFinalisasiNon;