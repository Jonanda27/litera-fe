...
export default function StepQuoteCollection({ formData, onDataChange }: StepQuoteProps) {
  ...
  return (
    <div className="space-y-6 pb-10 text-slate-800 relative">
      ...
      {/* HEADER SECTION */}
      <div className="flex justify-between items-center bg-gradient-to-r from-violet-600 to-purple-700 p-5 rounded-[2rem] shadow-lg border-b-4 border-violet-900/20">
        ...
        <button
          ...
          className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase transition-all shadow-md active:scale-95 flex items-center gap-2 ${
            isAdding ? "bg-rose-500 text-white shadow-rose-200" : "bg-white text-violet-700 hover:bg-violet-50"
          }`}
        >
                ...
                <div className="flex flex-wrap gap-2">
                  {sourceTypes.map((type) => (
                    <button key={type.label} type="button" onClick={() => setNewItem({...newItem, sourceType: type.label})}
                      className={`py-2 px-3 rounded-xl text-[9px] font-black uppercase transition-all border flex items-center gap-2 ${
                        newItem.sourceType === type.label ? "bg-violet-600 border-violet-600 text-white shadow-md" : "bg-slate-50 border-slate-100 text-slate-500 hover:border-violet-200"
                      }`}
                    >
        ...
        {selectedQuote && (
          ...
          >
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
              className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden text-slate-800"
      ...
      <style jsx>{`
        ...
        .form-input-modal:focus { background-color: white; border-color: #7c3aed; box-shadow: 0 4px 12px rgba(124, 58, 237, 0.05); }
