...
const SortableOutlineItem = ({ item, onChange }: { item: any, onChange: (id: string, field: string, value: string) => void }) => {
  const { 
    attributes, 
    listeners, 
    setNodeRef, 
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-start gap-4 bg-white border-2 p-5 rounded-2xl shadow-sm transition-all duration-300 ${
        isDragging ? "border-[#1E4E8C] shadow-xl scale-[1.02]" : "border-slate-100"
      } group hover:border-indigo-300 hover:shadow-md`}
    >
      <div 
        {...attributes} 
        {...listeners} 
      ...
      >
        <div className="bg-slate-50 group-hover:bg-indigo-50 p-2 rounded-lg transition-colors">
          <span className="text-slate-400 group-hover:text-[#1E4E8C] text-xl block">⠿</span>
        </div>
      </div>

      <div className="flex-1 space-y-3">
        <div className="relative">
          <label className="text-[8px] font-black text-indigo-400 uppercase tracking-widest block mb-1">Struktur Bab</label>
          <input
            type="text"
...
export default function StepPramenulis({
  ...
  useEffect(() => {
    const fetchPramenulisData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:4000/api/books/pramenulis/${bookId}`, {
          method: "GET",
          headers: { 
        ...
        if (response.ok && result.data) {
          ...
          if (d.ideCepat?.date) {
            setTanggal(d.ideCepat.date.split('T')[0]); 
          }

          if (d.moodBoards && d.moodBoards.length > 0) {
            setMoodBoards(d.moodBoards.map((mb: any) => ({
              id: mb.id,
          ...
          if (d.researches && d.researches.length > 0) {
            setResearches(d.researches.map((rs: any) => {
              const tagMatch = rs.notes?.match(/^\[Tag: (.*?)\]\n/);
              const cleanNotes = tagMatch ? rs.notes.replace(tagMatch[0], "") : rs.notes;
          ...
          if (d.outlines && d.outlines.length > 0) {
            setOutlineItems(d.outlines);
          }
        }
      } catch (err) {
  ...
  const addMoodBoardCard = () => {
    setMoodBoards([...moodBoards, { id: Date.now(), previewUrl: "", category: "Karakter" }]);
  };
  ...
  const updateMoodBoardCategory = (id: number, newCategory: string) => {
    setMoodBoards(moodBoards.map(mb => mb.id === id ? { ...mb, category: newCategory } : mb));
  };
  ...
  const handleImageUpload = (id: number, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setMoodBoards(prev => prev.map(mb => mb.id === id ? { ...mb, previewUrl: base64String } : mb));
      };
  ...
  const updateResearchItem = (id: string, field: string, value: string) => {
    setResearches(prev => prev.map(item => item.id === id ? { ...item, [field]: value } : item));
  };
  ...
  const handleResearchFileUpload = (id: string, event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
  ...
  const updateOutlineItem = (id: string, field: string, value: string) => {
    setOutlineItems((prev: any[]) => prev.map((item) => (item.id === id ? { ...item, [field]: value } : item)));
  };

  const filteredResearches = researches.filter((r) => {
    const q = searchQuery.toLowerCase();
    return (r.title?.toLowerCase().includes(q) || r.tag?.toLowerCase().includes(q) || r.notes?.toLowerCase().includes(q));
  });

  const testSimpanPramenulis = async () => {
    setIsLoading(true);
    try {
      const token = localStorage.getItem("token"); 
      if (!token) { alert("Token hilang! Silakan login kembali."); setIsLoading(false); return; }
      ...
      const activeOutlines = outlineItems.filter(ot => ot.title.trim() !== "");

      const payload = {
        bookId: bookId,
        title: formData.ideCepat || "Buku Baru",
        ideCepat: {
      ...
      const response = await fetch("http://localhost:4000/api/books/pramenulis", {
        method: "POST",
      ...
      if (response.ok) alert(`✅ Berhasil! Book ID: ${data.bookId}`);
  ...
  return (
    <div className="space-y-12 pb-20">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-2xl border-l-8 border-[#1E4E8C] shadow-sm relative overflow-hidden">
        <div className="absolute right-[-20px] top-[-20px] text-8xl opacity-5 grayscale">🖋️</div>
        <p className="relative z-10 text-[12px] font-medium text-indigo-900 leading-relaxed italic">
          "Ini tahap paling awal, sebelum nulis kata pertama. Fungsinya buat nangkep ide, nyimpen riset, dan nyusun kerangka."
        </p>
      </div>
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-center border-b-2 border-slate-100 pb-3">
          <div className="flex items-center gap-3">
            <span className="bg-indigo-600 text-white w-8 h-8 flex items-center justify-center rounded-lg shadow-lg shadow-indigo-200 text-xs font-bold">01</span>
            <h4 className="font-black text-sm uppercase tracking-wider text-slate-700">Catatan Ide Cepat</h4>
          </div>
          <span className="text-[9px] text-indigo-400 font-black uppercase tracking-[0.2em]">Capture Spark</span>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border-2 border-slate-50 shadow-xl shadow-slate-100/50 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-4">
            <div>
              <label className="text-[9px] font-black text-slate-400 uppercase mb-2 block tracking-widest">Judul Ide</label>
              <input 
                type="text" 
            ...
            <div>
              <label className="text-[9px] font-black text-slate-400 uppercase mb-2 block tracking-widest">Detail Ide</label>
              <textarea 
          ...
          <div className="space-y-5 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
            <div>
              <label className="text-[9px] font-black text-indigo-400 uppercase mb-2 block tracking-widest">Tanggal Capture</label>
              <input 
                type="date" 
                value={tanggal} 
            ...
            <div>
              <label className="text-[9px] font-black text-indigo-400 uppercase mb-2 block tracking-widest">Genre / Tag</label>
              <input 
                type="text" 
            ...
            <div className="pt-2">
              <div className="bg-indigo-100 p-3 rounded-xl text-[9px] text-indigo-700 font-bold leading-tight border border-indigo-200">
                💡 Tips: Jangan biarkan ide lewat begitu saja. Tulis apa adanya dulu!
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-center border-b-2 border-slate-100 pb-3">
          <div className="flex items-center gap-3">
            <span className="bg-pink-600 text-white w-8 h-8 flex items-center justify-center rounded-lg shadow-lg shadow-pink-200 text-xs font-bold">02</span>
            <h4 className="font-black text-sm uppercase tracking-wider text-slate-700">Mood Board Visual</h4>
          </div>
          <button 
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {moodBoards.map((mb) => (
            <div key={mb.id} className="bg-white p-3 rounded-3xl border-2 border-slate-50 shadow-lg hover:shadow-pink-100 transition-all group">
              <label className="aspect-[3/4] border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center bg-slate-50 hover:bg-pink-50 hover:border-pink-300 transition-all cursor-pointer relative overflow-hidden shadow-inner mb-3">
                ...
                {mb.previewUrl ? (
                  <>
                    <img src={mb.previewUrl} className="w-full h-full object-cover transition-transform group-hover:scale-105" alt="Preview" />
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                      <span className="text-white text-[9px] font-black uppercase border-2 border-white px-3 py-1.5 rounded-lg tracking-widest">Update</span>
                    </div>
                  </>
                ) : (
                  <div className="text-center p-4">
                    <div className="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center mx-auto mb-2 text-xl">🖼️</div>
                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-tight">Drop Image</span>
                  </div>
                )}
              </label>
              ...
              <div className="flex gap-2 items-center">
                <select 
                  value={mb.category} 
                  onChange={(e) => updateMoodBoardCategory(mb.id, e.target.value)} 
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-center border-b-2 border-slate-100 pb-3">
          <div className="flex items-center gap-3">
            <span className="bg-emerald-600 text-white w-8 h-8 flex items-center justify-center rounded-lg shadow-lg shadow-emerald-200 text-xs font-bold">03</span>
            <h4 className="font-black text-sm uppercase tracking-wider text-slate-700">Database Riset</h4>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-h-[600px] overflow-y-auto pr-3 custom-scrollbar">
          {filteredResearches.length === 0 ? (
            <div className="col-span-full py-20 bg-slate-50 border-4 border-dashed border-slate-100 rounded-3xl flex flex-col items-center opacity-40">
              <span className="text-5xl mb-4">📂</span>
              <p className="text-[11px] text-slate-500 font-black uppercase tracking-[0.3em]">Belum ada riset terkumpul</p>
            </div>
          ) : (
                ...
                <div className="p-6 space-y-5">
                  <div className="flex gap-4">
                    <div className="flex-1">
                      <label className="text-[8px] font-black text-emerald-500 uppercase tracking-widest block mb-2">Source Title</label>
                      <input 
                        type="text" 
                        placeholder="Nama sumber..." 
                        value={rs.title} 
                        onChange={(e) => updateResearchItem(rs.id, 'title', e.target.value)} 
                    ...
                    <div className="w-[100px]">
                      ...
                      <input 
                        ...
                        placeholder="#TAG" 
                        value={rs.tag} 
                        onChange={(e) => updateResearchItem(rs.id, 'tag', e.target.value)} 
                  ...
                  <div className="space-y-3">
                    ...
                    {rs.mode === 'link' ? (
                      <input 
                        ...
                        value={rs.linkUrl} 
                        onChange={(e) => updateResearchItem(rs.id, 'linkUrl', e.target.value)} 
                      ...
                      <label className="block text-center border-2 border-dashed border-emerald-100 p-4 text-[10px] cursor-pointer bg-emerald-50/30 rounded-2xl text-emerald-600 hover:bg-emerald-50 transition-all font-black uppercase tracking-widest">
                        ...
                        <input type="file" className="hidden" onChange={(e) => handleResearchFileUpload(rs.id, e)} />
                      </label>
                    )}
                  </div>

                  <div>
                    <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-2">Key Notes</label>
                    <textarea 
                      placeholder="Apa inti dari riset ini?" 
                      value={rs.notes} 
                      onChange={(e) => updateResearchItem(rs.id, 'notes', e.target.value)} 
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-center border-b-2 border-slate-100 pb-3">
          <div className="flex items-center gap-3">
            <span className="bg-indigo-900 text-white w-8 h-8 flex items-center justify-center rounded-lg shadow-lg text-xs font-bold">04</span>
            <h4 className="font-black text-sm uppercase tracking-wider text-slate-700">Penyusun Outline</h4>
          </div>
          <span className="text-[9px] text-slate-400 font-black uppercase tracking-[0.2em]">Logical Path</span>
        </div>
        ...
        <div className="bg-slate-50/50 p-6 rounded-[40px] border-2 border-white shadow-inner">
          ...
          <button 
            onClick={() => setOutlineItems([...outlineItems, { id: `ot-${Date.now()}`, title: "", sub1: "", sub2: "" }])} 
          ...
          >
            <span className="text-xl group-hover:scale-125 transition-transform">➕</span> Tambah Bab Baru
          </button>
        </div>
      </section>

      {/* TOMBOL INTEGRASI SIMPAN */}
      <div className="pt-10 border-t-2 border-slate-100 px-4">
        <button
          onClick={testSimpanPramenulis} 
          disabled={isLoading} 
          className={`w-full py-6 text-white font-black rounded-3xl uppercase transition-all shadow-2xl active:scale-[0.98] text-[12px] tracking-[0.3em] relative overflow-hidden group ${
            isLoading ? "bg-slate-400 cursor-not-allowed" : "bg-gradient-to-r from-emerald-600 to-teal-600 hover:shadow-emerald-200"
          }`}
        >
          {isLoading ? (
