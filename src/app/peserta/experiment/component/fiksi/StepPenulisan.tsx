...
export default function StepPenulisan({
  ...
  const handleReflow = useCallback(
    (index: number) => {
      ...
      // Cek apakah konten sudah melebihi batas tinggi elemen (overflow)
      if (el.scrollHeight > el.clientHeight) {
        ...
        // tanpa memindahkan isi (node/elemen) dari halaman saat ini
        setTimeout(() => {
          const nextPage = editorRefs.current[index + 1];
          if (nextPage) {
            nextPage.focus();
            
            // Opsional: pastikan kursor berada di posisi yang benar (awal halaman baru)
            const selection = window.getSelection();
            const range = document.createRange();
            range.selectNodeContents(nextPage);
            range.collapse(true); // true = kursor di awal
            selection?.removeAllRanges();
            selection?.addRange(range);
          }
        }, 50); // Jeda sedikit agar DOM selesai me-render halaman baru
      }
    },
    [pageCount],
  );
